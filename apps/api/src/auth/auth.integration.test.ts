...
if (
  ...
  throw new Error(
    "Run via npm run test:auth against the isolated auth database.",
  );
}
...
const sid = (value: string) =>
  decodeURIComponent(value.slice("dtp.sid=".length)).slice(2).split(".")[0]!;
...
async function setup() {
  ...
  auth.router.get(
    "/test/passenger",
    auth.requireAuth,
    auth.requireRole("PASSENGER"),
    (_req, res) => res.json({ data: "passenger" }),
  );
  auth.router.get(
    "/test/driver",
    auth.requireAuth,
    auth.requireRole("DRIVER"),
    (_req, res) => res.json({ data: "driver" }),
  );
...
describe("backend authentication with real PostgreSQL sessions", () => {
  ...
  it("binds CSRF to session and requires an allowed Origin", async () => {
    ...
    for (const [sentOrigin, sentCookie, sentToken] of [
      [origin, b.cookie, a.token],
      ["https://evil.invalid", a.cookie, a.token],
      ["", a.cookie, a.token],
    ]) {
      const response = await request(app)
        .post("/api/v1/auth/login")
        .set("Origin", sentOrigin!)
        .set("Cookie", sentCookie!)
        .set("X-CSRF-Token", sentToken!)
        .send(credentials);
      expect(response.status).toBe(403);
    }
    expect(
  ...
  it("registers a passenger, normalizes email/name, preserves password and stores only a hash", async () => {
    ...
    expect(await verifyPassword(password.trim(), user.password_hash)).toBe(
      false,
    );
    const authenticatedCookie = cookie(response);
    expect(authenticatedCookie).not.toBe(anonymous.cookie);
    expect(
      await db.session.findUnique({ where: { sid: sid(anonymous.cookie) } }),
    ).toBeNull();
    expect(
      (
        await request(app)
  ...
  it("rejects role injection, unknown fields and invalid input", async () => {
    ...
    for (const data of [
      ...
      { name: "", ...credentials },
      { name: "Test", ...credentials, email: "bad" },
  ...
  it("resolves concurrent duplicate normalized emails through the database constraint", async () => {
    ...
    expect(
      responses.find((value) => value.status === 409)!.body.error.code,
  ...
  it("logs in seeded passenger and driver, enforcing both role boundaries", async () => {
    for (const [name, role, allowed, denied] of [
      ["nusrat", "PASSENGER", "passenger", "driver"],
      ["jashim", "DRIVER", "driver", "passenger"],
    ]) {
      const authenticated = await login({
        ...credentials,
        email: name + "@demo.dhaka.test",
      });
      expect(authenticated.response.body.data.role).toBe(role);
      expect(Object.keys(authenticated.response.body.data).sort()).toEqual([
        "email",
        "id",
        "name",
        "role",
      ...
      expect(
        (
          await request(app)
            .get("/api/v1/auth/test/" + denied)
            .set("Cookie", authenticated.cookie)
        ).status,
      ).toBe(403);
    }
    expect((await request(app).get("/api/v1/auth/test/driver")).status).toBe(
      401,
    );
  }, 15000);
  ...
  it("returns identical generic errors for unknown email and wrong password", async () => {
    ...
    const errors = [];
    for (const data of [
      { ...credentials, password: "WrongPassword2026" },
      { ...credentials, email: "missing@test.invalid" },
    ]) {
      const response = await request(app)
        .post("/api/v1/auth/login")
        .set("Origin", origin)
  ...
  it("rotates session/token at login; old token fails and a freshly fetched token allows logout", async () => {
    ...
    expect(
      await db.session.findUnique({
        where: { sid: sid(authenticated.anonymous.cookie) },
      }),
    ).toBeNull();
    expect(
      (
        await request(app)
          .get("/api/v1/auth/me")
          .set("Cookie", authenticated.anonymous.cookie)
      ).status,
    ).toBe(401);
    const stale = await request(app)
      ...
      .set("Origin", origin)
      .set("Cookie", authenticated.cookie)
      .set("X-CSRF-Token", authenticated.anonymous.token);
    ...
    expect(result.headers["set-cookie"][0]).toMatch(
      /dtp.sid=; Path=\/; Expires=Thu, 01 Jan 1970.*HttpOnly.*SameSite=Lax/,
    );
    expect(
      await db.session.findUnique({
        where: { sid: sid(authenticated.cookie) },
      }),
    ).toBeNull();
    expect(
      (
  ...
  it("persists a fixed eight-hour session across API/store reconstruction", async () => {
    ...
    expect(stored.expire.getTime() - Date.now()).toBeGreaterThan(
      AUTH_LIFETIME_MS - 10000,
    );
    expect(stored.expire.getTime() - Date.now()).toBeLessThanOrEqual(
      AUTH_LIFETIME_MS + 1000,
    ); // store rounds up to whole seconds
    expect(
      (stored.sess as { authExpiresAt: number }).authExpiresAt - Date.now(),
    ).toBeLessThanOrEqual(AUTH_LIFETIME_MS);
    await auth.close();
    await setup();
    const response = await request(app)
      .get("/api/v1/auth/me")
    ...
    expect(response.headers["set-cookie"]).toBeUndefined();
    expect(
      (await db.session.findUniqueOrThrow({ where: { sid: stored.sid } }))
        .expire,
    ).toEqual(stored.expire);
  }, 15000);
  ...
  it("rejects expired DB sessions even when a client replays the cookie and prunes expired rows", async () => {
    ...
    await db.session.update({
      where: { sid: id },
      data: { expire: new Date(Date.now() - 60000) },
    });
    expect(
      (
        await request(app)
    ...
    await new Promise<void>((resolve, reject) =>
      auth.store.pruneSessions((error) => (error ? reject(error) : resolve())),
    );
  ...
  it("rejects application expiry even if store expiry was extended", async () => {
    ...
    await db.session.update({
      where: { sid: id },
      data: {
        sess: { ...(row.sess as object), authExpiresAt: Date.now() - 1000 },
      },
    });
    expect(
      (
  ...
  it("sets secure cookies only over deliberately trusted HTTPS proxy configuration", async () => {
    ...
    try {
      ...
      expect(
        (await request(target).get("/api/v1/auth/csrf")).headers["set-cookie"],
      ).toBeUndefined();
    } finally {
      await secure.close();
    }
    expect(() =>
      loadAuthConfig({
        ...
        AUTH_ORIGINS: "http://public.example",
        SESSION_COOKIE_SECURE: "false",
      }),
    ).toThrow();
    expect(() =>
      loadAuthConfig({
        SESSION_SECRET: "replace-with-a-secret-at-least-32-chars",
      }),
    ).toThrow();
    expect(
      loadAuthConfig({
  ...
  it("fails closed with a generic server error when the session store cannot be read", async () => {
    ...
    const get = vi
      .spyOn(auth.store, "get")
      .mockImplementation((_id, callback) => {
        callback(new Error("private-database-details"));
      });
    try {
      const response = await request(app)
        .get("/api/v1/auth/me")
  ...
  it("rechecks account role from PostgreSQL rather than trusting a stale session role", async () => {
    ...
    expect(
      (
        await request(app)
          .get("/api/v1/auth/test/passenger")
