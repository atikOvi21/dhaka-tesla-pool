...
const registerInput = loginInput
...
export async function createAuth(options: AuthOptions) {
  ...
  const store = new PgStore({
    ...
    errorLog: () =>
      console.error(JSON.stringify({ event: "session_store_failed" })),
  });
  pool.on("error", () =>
    console.error(JSON.stringify({ event: "session_pool_failed" })),
  );
  const cookie = {
    path: "/",
    httpOnly: true,
  ...
  const csrf: RequestHandler = (req, _res, next) => {
    ...
    if (
      !options.origins.includes(req.get("Origin") ?? "") ||
      ...
      throw new AuthError(
        403,
        "CSRF_INVALID",
        "Refresh the security token and try again.",
      );
    }
    next();
  };
  ...
  const requireRole =
    (role: SafeUser["role"]): RequestHandler =>
    (req, _res, next) => {
      if (!req.currentUser)
        throw new AuthError(401, "UNAUTHENTICATED", "Sign in to continue.");
  ...
  const credentialsLimiter = limited(
    options.credentialLimit ?? 10,
    15 * 60 * 1000,
  );
  const router = Router();
  ...
  router.get(
    "/csrf",
    limited(options.bootstrapLimit ?? 60, 15 * 60 * 1000),
    async (req, res) => {
  ...
  router.post("/register", credentialsLimiter, async (req, res) => {
    ...
    if (!input.success)
      throw new AuthError(
        400,
        "VALIDATION_ERROR",
        "Use a valid email, name (1–100 characters), and password (12–128 characters); extra fields are not allowed.",
      );
  ...
  router.post("/login", credentialsLimiter, async (req, res) => {
    ...
    if (!input.success)
      throw new AuthError(
        400,
        "VALIDATION_ERROR",
        "Use a valid email and password (12–128 characters); extra fields are not allowed.",
      );
  ...
  router.get("/me", requireAuth, (_req, res) =>
    res.json({ data: _req.currentUser }),
  );
  ...
  return {
    middleware: [sessionMiddleware, expire, csrf],
    router,
    requireAuth,
    requireRole,
