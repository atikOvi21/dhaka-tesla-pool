...
if (
  !target ||
  new URL(target).pathname !== "/dhaka_tesla_auth_test" ||
  (process.env.DATABASE_URL &&
    new URL(process.env.DATABASE_URL).pathname === new URL(target).pathname)
)
  throw new Error(
    "Use the isolated dhaka_tesla_auth_test database, never development.",
  );
const env = {
  ...process.env,
  DATABASE_URL: target,
